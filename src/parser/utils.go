...
// Dump dumps the AST defined by `node` as a list of sexps. Returns a string
...
func splitCommand(line string) (string, []string, string, error) {
	...
	cmdline := tockenWhitespace.Split(strings.TrimSpace(line), 2)
...
func stripComments(line string) string {
	...
	if tockenComment.MatchString(line) {
		return tockenComment.ReplaceAllString(line, "")
	}
